...
export const PixiStaticMap = PixiComponent('StaticMap', {
  create: (props: { map: WorldMap; [k: string]: any }) => {
    ...
    const resolveUrl = (u: string) => {
      ...
      if (u.startsWith('/')) return u; // absolute to domain root
      const base = import.meta.env.BASE_URL || '/';
      return base + u.replace(/^\/+/, '');
    };

    const bt = PIXI.BaseTexture.from(resolveUrl(map.tileSetUrl), {
    ...
    for (const [sheet, sprites] of spritesBySheet.entries()) {
      ...
      const texture = PIXI.BaseTexture.from(resolveUrl(url), {
        scaleMode: PIXI.SCALE_MODES.NEAREST,
      });
